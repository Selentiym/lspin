program test
    ...
    call testFuncDerivIntegralsAssymetry()

contains

    subroutine testFuncDerivIntegralsAssymetry()
        real(WP), allocatable::poly1(:),poly2(:), derivTest(:,:), w1(:), x1(:)
        real(WP)::kappa, Z, summa, gammaRel1
...
!                    - 1/2 * laguerrePoly(k, j) )
                ...
                !derivTest(i,j) = summa
                derivTest(i,j) = integrateOnGrid(lagVector(j, 'U'),lagDerivativeVector(i, 'U'))
            end do
        end do
    ...
    end subroutine testFuncDerivIntegralsAssymetry
